...
// Draw the penality for distance from PMT
// Change penalty slope
struct Vec3 {
  double x, y, z;
  Vec3() : x(0), y(0), z(0) {}
...
static inline double epsCoupleExp(double d, double eps0, double lambdaC) {
...
double ToyScintOptics(
  ...
  int maxSteps = 2000,
double rPMT = 2.5,    // 2" PMT radius ~2.5 cm
double epsCouple = 0.9,   // light guide + grease + window transmission
double pde = 0.2   // PMT PDE (QE * collection)
){
//  TRandom3 rng(0);
  ...
  for (long long i = 0; i < NperPoint; i++) {
    ...
    for (int step = 0; step < maxSteps; step++) {
      ...
      if (plane == 0) { // x=0
//	  double d = (plane == 0) ? xEmit : (L - xEmit);
	  double d=sqrt(pos.y * pos.y + pos.z * pos.z );
	  double eps = 1;
	  if (d>rPMT){
	  	  eps = epsCoupleExp(d, 0.90, 120.0);      // eps0=0.90, lambdaC=120 cm
	}
	  double pend = eps * pde;
	  if (rng.Uniform()	< pend) nReach0++;
	  endReached=1;
	/*
        bool inPMT = InsidePMTCircle(pos.y, pos.z, rPMT);
	if (inPMT) {
	        endReached = 1;
        	nReach0++;
...
*/
  ...
  double f0 = double(nReach0) / double(NperPoint);
  double fL = double(nReachL) / double(NperPoint);
  double fTot = double(nReach0 + nReachL) / double(NperPoint);
...
void ScanToyOptics(
  ...
  double T = 1.0,           // thickness (cm)
  double nScint = 1.58,
  double nOut = 1.0,
  double absLen = 300.0,    // cm
  double Rwrap = 0.95,
  int maxSteps = 2000
){
  ...
  // x axis: width W (cm), y axis: length L (cm)
  hFrac = new TH2D(
    "hFrac",
    "Toy fraction reaching an end;Width W (cm);Length L (cm)",
  ...
  hFracCost1 = new TH2D(
    "hFracCost1",
    "Toy (fraction * W * L/cost1pmt);Width W (cm);Length L (distance between PMT) (cm)",
    nBins, minCM - stepCM/2.0, maxCM + stepCM/2.0,
    nBins, minCM - stepCM/2.0, maxCM + stepCM/2.0
  );
  ...
  hFracCost2 = new TH2D(
    "hFracCost2",
    "Toy (fraction * W * L/cost 2pmts);Width W (cm);Length L (cm)",
    nBins, minCM - stepCM/2.0, maxCM + stepCM/2.0,
    nBins, minCM - stepCM/2.0, maxCM + stepCM/2.0
  );
  ...
  for (int ix = 1; ix <= nBins; ix++) {
    ...
    for (int iy = 1; iy <= nBins; iy++) {
      ...
      double frac =  ToyScintOptics(rng, NperPoint, L, W, T, nScint, nOut, absLen, Rwrap, maxSteps);
      hFrac->SetBinContent(ix, iy, frac);
      hFracCost1->SetBinContent(ix, iy, cost1/((frac/2)*W*L));
      hFracCost2->SetBinContent(ix, iy, cost2/(frac*W*L));

      std::cout << "W=" << W << " cm, L=" << L << " cm -> f=" << frac << "\n";
    }
  }
